function [motion_vector, avg_MAD, num_compare] = motionEstimationByLS(reference, curr, block_size, search_range)
% Computes motion vectors using Logarithmic Search
%
% Input
...
for r = 1:block_size:row-block_size+1
    for c = 1:block_size:col-block_size+1
        ...
        step_size = 2^(round(log(search_range))-1); % reference the search strat paper
        searched_coordintates = 
        while step_size > 1
           % process
           % establish a hashmap of seen coordintates, excluding middle
           % array of coordinates to see, if hits/exceed boundary, half
           % search_range
           % 
           % search each of the options, if best is at middle, half search
           % range, otherwise, step and search again, adding the current
           % middle coord to hashmap (and the other ones except next middle)
           % 
           % 
           % 
           % 
